...
export const PageForm:React.FC<PageFormProps>  = function PageForm ({originalItem, onSubmit,buttonName}){
    ...
    return(
        <Paper sx={{ padding: '4'}} elevation={3}>
                <Box padding={4}>
                    <Typography data-testid='page_form_title' variant="h3" align="center">Editing a Todo</Typography>
                    <Box m={3} display="flex" justifyContent='space-evenly' alignItems="center">
                        <TextField required id="title-input" label="Title" variant="outlined" sx = {{width:'35%'}} value={titleValue} onChange={(e)=>{setTitleValue(e.target.value)}}/>
                        <TextField id="duedate-input" label="Due Date" variant="outlined" sx = {{width:'35%'}} value={duedateValue} onChange={(e)=>{setDuedateValue(e.target.value)}}/>
                    </Box>
